import { assertIsDefined } from "../datatypes/nullish";
import { assertIsInstanceOf } from "../datatypes/objects";
...
describe("tryCatchAsync", () => {
  ...
  it("expected stack for error", async () => {
    ...
    expect(stackSplited[0]).toContain(ERROR_MESSAGE);
    expect(stackSplited[1]).toContain(__filename);
    expect(stackSplited[2]).toContain("jest");
  } );
} );
...
describe("tryCatchLogErrorAsync", () => {
  ...
  it("expected stack for error", () => {
    ...
    expect(stackSplited[0]).toContain(ERROR_MESSAGE);
    expect(stackSplited[1]).toContain(__filename);
    expect(stackSplited[2]).toContain("jest"); // interno de jest
  } );
} );
